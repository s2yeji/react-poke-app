...
const DetailPage = () => {
    ...
    try {
      ...
      if (pokemonData) {
        const { name, id, types, weight, height, stats, abilities, sprites } =
          pokemonData;
        // console.log(name, id, types, weight, height, stats, abilities, sprites);
        // console.log(sprites);
  ...
  const formatPokemonSprites = (sprites) => {
    const newSprites = { ...sprites }; // 원본을 복사해서 새롭게 만드는 작업으로 원본이 변경되지 않는다.
  ...
  const formatPokemonAbilities = (abilities) => {
    return abilities
      .filter((_, index) => index <= 1) // ability를 _로 작성한 이유는? 사용을 안 하기 때문
